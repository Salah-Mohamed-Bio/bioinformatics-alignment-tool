"""
Flask API for sequence alignment.
Wraps alignment_logic.py in a minimal, deployable REST API.
"""
...
def parse_multi_fasta(text):
    ...
    for line in text.strip().splitlines():
        if line.startswith(">"):
            ...
            current_id = line[1:].strip().split()[0]  # take first word as ID
            current_seq_parts = []
        else:
            clean = re.sub(r"[^A-Za-z]", "", line)
            if clean:
...
def align_batch():
    ...
    Multipart form fields:
        file   - multi-FASTA file with multiple sequences
